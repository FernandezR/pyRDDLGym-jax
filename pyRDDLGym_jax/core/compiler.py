...
class JaxRDDLCompiler:
    ...
    def _compile_reward(self, aux):
        ...
        requirements = self.rddl.ast.domain.requirements if self.rddl.ast else []
        use_vector_reward = 'reward-vector' in requirements

        if use_vector_reward:
            return self._compile_vector_reward(aux)
        else:
            return self._jax(self.rddl.reward, aux, dtype=self.REAL)

    def _compile_vector_reward(self, aux):
        """Compile reward as a vector (per-agent rewards) instead of scalar.

        This function expects the reward expression to be a sum over agents:
            reward = [sum_{?a : agent} [...individual agent reward...]]

        It compiles the inner expression for each agent separately, returning
        a vector of rewards indexed by agent.
        """
        NORMAL = JaxRDDLCompiler.ERROR_CODES['NORMAL']
        reward_expr = self.rddl.reward

        # Check if reward is a sum aggregation
        ...
        if etype != 'aggregation' or op != 'sum':
            raise RDDLNotImplementedError(
                ...
                f'but got {etype} {op}.\n' + print_stack_trace(reward_expr))

        # Extract the inner expression and the variable being summed over
        * captured_vars, inner_expr = reward_expr.args
        ...
        # captured_vars is a list of tuples: [('?a', 'agent')]
        # Extract the variable name and type
        agent_var, agent_type = captured_vars[0]

        # Compile the inner expression (per-agent reward)
        jax_inner = self._jax(inner_expr, aux, dtype=self.REAL)

        # The compiled inner expression will have shape (..., n_agents)
        # where n_agents is the number of agents
        # We don't need to do anything special - just return it directly
        # The aggregation compilation already handles the vectorization

        def _jax_wrapped_vector_reward(x, aux, key):
            # x is a tuple (fls, nfls) from the env wrapper
            # Unpack it and call the inner function with proper signature
            fls, nfls = x
            sample, key, err, aux = jax_inner(fls, nfls, aux, key)
            # sample has shape (..., n_agents) - one reward per agent
            return sample, key, err, aux

        return _jax_wrapped_vector_reward
