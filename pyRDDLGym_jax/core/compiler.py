...
class JaxRDDLCompiler:
    ...
    def _compile_reward(self, init_params):
        ...
        if use_vector_reward:
            return self._compile_vector_reward(init_params)
        else:
            return self._jax(self.rddl.reward, init_params, dtype=self.REAL)

    def _compile_vector_reward(self, init_params):
        ...
        * captured_vars, inner_expr = reward_expr.args
        ...
        jax_inner = self._jax(inner_expr, init_params, dtype=self.REAL)
        ...
        def _jax_wrapped_vector_reward(x, params, key):
            # The inner expression is already vectorized over agents
            sample, key, err, params = jax_inner(x, params, key)
